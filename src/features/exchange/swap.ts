...
const getFunction = async (
  ...
  try {
    ...
    const amountInAllDex = chainId === [ChainId.KOVAN] ? BigNumber.from(amountIn).sub(amountInKyper).sub(amountInUniV3).sub(amountInBalancer) : BigNumber.from(amountIn).sub(amountInKyper).sub(amountInUniV3);
