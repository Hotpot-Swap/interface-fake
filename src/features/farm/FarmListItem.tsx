...
const FarmListItem = ({ farm, farmLength }) => {
  ...
  const getApyForToken = useCallback (async (token: string, tokenPerYear: BigNumber) => {    
    ...
    const apy = tokenPriceVsQuote.times(yearlySTANDRewardAllocation).div(lpTotalInQuoteToken).times(100);
    const resultApy = !apy.isNaN() ? apy : new BigNumber(0);
    return resultApy.toNumber();
  }, [balanceNative]);

  const getRef = async () => {
    // TODO: call be to receive `ref`
    return ZERO_ADDRESS;
  }
  ...
  let totalDataAPY = `<p></p><p></p>${dataApyTken0 !== 0 ? `<div>${(dataApyTken0).toFixed(2)}%  - ${farm.pair.token.symbol} Farming APY</div>` : ''}${dataApyTken1 !== 0 ? `<div>${handleFarmingAPYDecimal(dataApyTken1)}% - ${farm.pair.quoteToken.symbol} Farming APY</div>` : ''}${dataApyTokenStand !== 0 ? `<div>${handleFarmingAPYDecimal(dataApyTokenStand)}% - STAND Farming APY</div>` : '0%'}`;
                  ...
                  <NewTooltip
                    ...
                    dataValue={
                      farm.type === FarmTypeEnum.TOKEN
                        ...
                        : `${handleAdvancedDecimal((dataApyTken0 + dataApyTken1 + dataApyTokenStand),6)}%`
                    }
                  />
                  </NumberTotal>
                  <div>
